...
server.listen(0, async () => {
  ...
  for (const adapter of ["http", "fetch"]) {
    try {
      await axios.request({ method: "get", url, params: circ, adapter, timeout: 3000 });
    } catch (e) {
      console.log("request()直调", adapter, ": isAxiosError=" + !!e.isAxiosError, "| 有config=" + !!(e && e.config), "| code=" + e.code);
    }
    try {
      await axios(url, { params: circ, adapter, timeout: 3000 });
    } catch (e) {
      console.log("axios()直调  ", adapter, ": isAxiosError=" + !!e.isAxiosError, "| 有config=" + !!(e && e.config), "| code=" + e.code);
    }
  }
  server.close();
});
