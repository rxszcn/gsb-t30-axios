...
// Config materialization can fail before dispatch (e.g. a circular structure
// in params that cannot be deep-merged). Surface such failures as an
// AxiosError carrying the original config, consistent with adapter-level
// params serialization failures.
const mergeRequestConfig = (defaults, config) => {
  try {
...
class Axios {
  ...
  _request(configOrUrl, config) {
    ...
    config = mergeRequestConfig(this.defaults, config);
...
utils.forEach(['delete', 'get', 'head', 'options'], function forEachMethodNoData(method) {
  ...
  Axios.prototype[method] = function (url, config) {
    ...
    try {
      mergedConfig = mergeRequestConfig(config || {}, {
        method,
        url,
