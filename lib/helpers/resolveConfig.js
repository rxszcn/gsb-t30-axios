...
function resolveConfig(config) {
  ...
  const fullPath = buildFullPath(baseURL, url, allowAbsoluteUrls, newConfig);

  try {
    newConfig.url = buildURL(fullPath, own('params'), own('paramsSerializer'));
  } catch (err) {
    // Params serialization failures (default serializer or a user-supplied
    // one) must surface as AxiosError carrying the request config, matching
    // the Node http adapter's own buildURL guard.
