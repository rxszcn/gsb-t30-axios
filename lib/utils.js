...
function merge(...objs) {
  ...
  // Plain-object sources currently being merged by an ancestor call. A value
  // that points back to one of them (circular structure, e.g. self-referencing
  // `params`) is assigned by reference instead of recursing until the stack
  // blows, so the circularity surfaces where the value is actually consumed
  // (e.g. URL params serialization) as a proper, wrappable error.
  const merging = (isContextDefined(this) && this.merging) || new Set();
  const result = {};
  const assignValue = (val, key) => {
    ...
    if (isPlainObject(existing) && isPlainObject(val)) {
      result[targetKey] = merging.has(val) ? val : merge.call({ merging }, existing, val);
    } else if (isPlainObject(val)) {
      result[targetKey] = merging.has(val) ? val : merge.call({ merging }, {}, val);
    } else if (isArray(val)) {
  ...
  for (let i = 0, l = objs.length; i < l; i++) {
    ...
    const track = isPlainObject(source);

    if (track) {
      if (merging.has(source)) {
        continue;
      }
